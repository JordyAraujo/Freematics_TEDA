...
public:
    ...
    virtual float calcMean(x)
    {
        float new_mean = ((k - 1) / k) * mean + (1 / k) * k;
        return new_mean;
    }
    virtual float calcVariance(x)
    {
        float distance_squared = pow((x - mean), 2);
        float new_var = ((k - 1) / k) * variance + distance_squared * (1 / (k - 1));
        return new_var;
    }
    virtual float calcEccentricity(x)
    {
        new_ecc = (1 / k) + (pow((mean - x), 2)) / k * variance));
    }
    virtual bool eval_point(x)
    {
        ...
        else
        {
            ...
            outlier_threshold = (pow(n, 2) + 1) / (2 * k);
